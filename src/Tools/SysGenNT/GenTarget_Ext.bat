...
echo del %PROJECT_ROOT%\GenTools\global.inc /F
echo del %PROJECT_ROOT%\GenTools\global.inc /F                                                           >> %LSG_LOGFILE%
     del %PROJECT_ROOT%\GenTools\global.inc /F                                                   >> nul 2>> %LSG_LOGFILE%
if errorlevel 1 goto error

echo copy %PROJECT_ROOT%\GenTools\global_Ext.inc %PROJECT_ROOT%\GenTools\global.inc
echo copy %PROJECT_ROOT%\GenTools\global_Ext.inc %PROJECT_ROOT%\GenTools\global.inc                       >> %LSG_LOGFILE%
     copy %PROJECT_ROOT%\GenTools\global_Ext.inc %PROJECT_ROOT%\GenTools\global.inc               >> nul 2>> %LSG_LOGFILE%
if errorlevel 1 goto error
...
echo del %PROJECT_ROOT%\Target\%LSG_TARGET%\Setup\4CWinAddOn.ism /F
echo del %PROJECT_ROOT%\Target\%LSG_TARGET%\Setup\4CWinAddOn.ism /F                             >> %LSG_LOGFILE%
     del %PROJECT_ROOT%\Target\%LSG_TARGET%\Setup\4CWinAddOn.ism /F                     >> nul 2>> %LSG_LOGFILE%
if errorlevel 1 goto error

echo copy %PROJECT_ROOT%\Target\%LSG_TARGET%\Setup\4CWinAddOn_Ext.ism %PROJECT_ROOT%\Target\%LSG_TARGET%\Setup\4CWinAddOn.ism
echo copy %PROJECT_ROOT%\Target\%LSG_TARGET%\Setup\4CWinAddOn_Ext.ism %PROJECT_ROOT%\Target\%LSG_TARGET%\Setup\4CWinAddOn.ism                             >> %LSG_LOGFILE%
     copy %PROJECT_ROOT%\Target\%LSG_TARGET%\Setup\4CWinAddOn_Ext.ism %PROJECT_ROOT%\Target\%LSG_TARGET%\Setup\4CWinAddOn.ism                     >> nul 2>> %LSG_LOGFILE%
if errorlevel 1 goto error
...
echo perl ..\..\GenTools\make.pl %1_Ext
echo perl ..\..\GenTools\make.pl %1_Ext                                                         >> %LSG_LOGFILE%
     perl ..\..\GenTools\make.pl %1_Ext                                                 >> nul 2>> %LSG_LOGFILE%
if errorlevel 1 goto error

echo nmake -nologo -i -f %1_Ext.mak makemake "DEBUG=0" all 2>&1 | perl ..\..\GenTools\LogFile.pl Build.log
echo nmake -nologo -i -f %1_Ext.mak makemake "DEBUG=0" all 2>&1 | perl ..\..\GenTools\LogFile.pl Build.log          >> %LSG_LOGFILE%
     nmake -nologo -i -f %1_Ext.mak makemake "DEBUG=0" all 2>&1 | perl ..\..\GenTools\LogFile.pl Build.log  >> nul 2>> %LSG_LOGFILE%
if errorlevel 1 goto error
...
echo nmake -nologo -i -f %1_Ext.mak makemake "DEBUG=1" all 2>&1 | perl ..\..\GenTools\LogFile.pl Build.log
echo nmake -nologo -i -f %1_Ext.mak makemake "DEBUG=1" all 2>&1 | perl ..\..\GenTools\LogFile.pl Build.log          >> %LSG_LOGFILE%
     nmake -nologo -i -f %1_Ext.mak makemake "DEBUG=1" all 2>&1 | perl ..\..\GenTools\LogFile.pl Build.log  >> nul 2>> %LSG_LOGFILE%
if errorlevel 1 goto error

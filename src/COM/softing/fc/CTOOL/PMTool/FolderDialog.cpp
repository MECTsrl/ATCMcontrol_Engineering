/*H>> $Header: /4CReleased/V2.20.00/COM/softing/fc/CTOOL/PMTool/FolderDialog.cpp 1     28.02.07 19:01 Ln $
 *----------------------------------------------------------------------------*
 *
 * =FILENAME            $Workfile: FolderDialog.cpp $
 *                      $Logfile: /4CReleased/V2.20.00/COM/softing/fc/CTOOL/PMTool/FolderDialog.cpp $
 *
...
CFolderDialog::CFolderDialog(LPCTSTR szInitPath, LPCTSTR szMsgTitle) : CFileDialog(TRUE, NULL, DIR_MASK)
{
    ...
    if(m_sSelPath.IsEmpty())
    {
        ::GetCurrentDirectory(_MAX_PATH, m_sSelPath.GetBuffer(_MAX_PATH));
        m_sSelPath.ReleaseBuffer();
    }

    if(m_sSelPath.GetLength() > 0)
    {
 ...
 *  if not, strip path until existing part found
 *  if drive is not valid, path is not changed
 *
 * @param           rstrPath: path (in/out)
 * @return          existing part found?
 * @exception       -
 * @see             -
*/
BOOL CFolderDialog::GetExisting(CString& rstrPath)
{
    CString strPath(rstrPath);
    CFileStatus tFileStatus;
    if(CFile::GetStatus(strPath, tFileStatus) && tFileStatus.m_attribute & CFile::directory)
    {
        return TRUE;
    }
    BOOL    bFound = FALSE;
    while(StripPath(strPath))
    {
        if(CFile::GetStatus(strPath, tFileStatus) && tFileStatus.m_attribute & CFile::directory)
        {
            rstrPath = strPath;
...
BOOL CFolderDialog::StripPath(CString& rstrPath)
{
    int iFound = rstrPath.ReverseFind(_T('\\'));
    if(iFound == -1)
    {
        return FALSE;
    }
    rstrPath = rstrPath.Left(iFound);
    return TRUE;
}
 ...
 * 
 * *****************  Version 2  *****************
 * User: Sis          Date: 14.04.04   Time: 14:20
 * Updated in $/4Control/COM/softing/fc/CTOOL/PMTool
 * prevent initial directory to be empty. This causes a deadlock in some
 * cases
 * 
 * *****************  Version 1  *****************

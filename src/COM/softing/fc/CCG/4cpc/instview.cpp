...
)
{
    ...
    for(j=0; j<pRes->nTasks; j++)
    {
        ...
        //acyclic task, no interval:=
        bool            bACTask = pIF->lookUpTaskInit(pTsk,  CG_TASK_INTERVAL)==NULL;
        ...
        if(controlVer>=22000)
        {
            dmp<<_T("      STATETEXT_VAR= \"");
            dmp.dumpAsXMLUTF8(ipathState, false)<<_T(".")<<pTsk->pszId<<_T(".__statetext0\"\n");
            if(bACTask)
            {
                dmp<<_T("      USER_ONLCMD0 =\"End Wait|");
                dmp<<ipath<<'.'<<pTsk->pszId<<_T(".__usrcommand|STRING|'endwait'\"\n");
            }
        }

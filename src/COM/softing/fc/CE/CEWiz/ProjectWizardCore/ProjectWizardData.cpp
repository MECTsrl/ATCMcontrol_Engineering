...
HRESULT CProjectWizardData::CreateResourceFile (CProjectWizardResource *pResource)
{
    ...
    strFilename.Format ("%s." FILE_EXT_RESOURCE, pResource->GetResource ());
...
CString CProjectWizardData::BuildTaskStatement (CProjectWizardTask *pTask)
{
    ...
    strStatement.Format (_T ("TASK %s(INTERVAL:=T#100ms, PRIORITY:=8, AUTOSTART:=TRUE);"), pTask->GetName ());
    return (strStatement);
}
...
CString CProjectWizardData::BuildTargetXML (CProjectWizardResource *pResource)
{
    ...
    strText.Format (_T ("<PC CONTROL=\"%s\" CONSOLE=\"%s\" VERSION=\"22000\" RESOURCE=\"%s\" >\r\n</PC>\r\n"),
                    pResource->GetAddress (),
                    pResource->GetAddress (),
                    pResource->GetResource ());
    return (strText);
}
...
HRESULT CProjectWizardData::CreateResourceDependentFiles (CProjectWizardResource *pResource)
{
    ...
    hr = CreateGlobalVariableFile (pResource->GetResource ());
    if (hr != S_OK)
        return (hr);

    hr = CreateConstantFile (pResource->GetResource ());
    if (hr != S_OK)
        return (hr);

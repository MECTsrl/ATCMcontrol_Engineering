...
public:
    ...
    int  InternalPaintWithOffset (CDC *pDC, CRect drawArea, bool bIsBlank, const CPoint& offs);
...
public:
    ...
    bool FindText(SearchDat *pSD);
    void SelectSearchText(int Line, int Offset, int Length);
...
protected:
    ...
    virtual void SubClassDraw (CDC *pDC, CRect drawArea, const CPoint& offs);
...
protected:
    ...
    //}}AFX_MSG
    afx_msg void OnChar(UINT nChar, UINT nRepCnt, UINT nFlags); // leave STActions by keyboard
...
private:
    ...
    void DrawWatchpoints (CDC *pDC, CRect drawArea, const CPoint& offs);
    void DrawWatchpoint (CDC *pDC, CRect drawArea, CWatchpoint *pWP, const CPoint& offs);
...
inline CGrEditorDoc* CSTEditorView::GetDocument()

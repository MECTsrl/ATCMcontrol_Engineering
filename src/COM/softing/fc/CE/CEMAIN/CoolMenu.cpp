...
void CToolbarData::SetToolbarId(UINT uiID)
{
    m_uiID = uiID;
}

void CToolbarData::SetBackgroundColor(COLORREF bgColor)
{
    m_bgColor = bgColor;
}

void CToolbarData::AddToolbarButton(UINT uiItemID, const CString& rstrBitmapFileName)
{
    m_ItemIdArray.Add(uiItemID);
    m_astrBitmapFileName.Add(rstrBitmapFileName);
}

int CToolbarData::GetNumButtons()
{
...
{
	int	iNumAccel = m_mapIDtoAccel.GetCount();
	for(int iAccel = 0; iAccel < iNumAccel; ++iAccel)
	{
		delete m_mapIDtoAccel[iAccel];
	}
...
//------------------------------------------------------------------*
/**
 * destroy runtime toolbar data.
 *
*/
void CCoolMenuManager::DestroyRuntimeToolbarData()
{
    CToolbarData*   pToolbarData;
    POSITION    pos = m_tToolbarDataList.GetHeadPosition();
    while(pos)
    {
        pToolbarData = m_tToolbarDataList.GetNext(pos);
        ASSERT(pToolbarData);
        delete pToolbarData;
    }
    m_tToolbarDataList.RemoveAll();
}
...
BOOL CCoolMenuManager::LoadToolbar(UINT nIDToolbar)
{
    ...
    // if toolbar has not been loaded from resources:
    // load bitmap from resource
    hbmToolbar = PxLib::LoadSysColorBitmap(nIDToolbar);
    // if bitmap not loaded -> return FALSE
	if (!hbmToolbar)
    {
    ...
    if(LoadToolbarFromStruct(nIDToolbar, ptbd, hbmToolbar))
    {
        return TRUE;
    }
    return FALSE;
...
//------------------------------------------------------------------*
/**
 * get runtime toolbar data.
 *
 *  for refresh
 *
 * @param           [in] nID: toolbar id
 * @return          pointer to toolbar data or NULL if not found
 *
*/
CToolbarData* CCoolMenuManager::GetRuntimeToolbarData(UINT nIDToolbar)
{
    ...
    while(pos)
    {
        pData = m_tToolbarDataList.GetNext(pos);
        if(pData->m_uiID == nIDToolbar)
        {
            break;
        }
    }
...
// SIS, 07.05.01: extracted resource free part of LoadToolbar() for use with KAD toolbardescription
//------------------------------------------------------------------*
/**
 * load toolbar from structure.
 *
 * @param           [in] nIDToolbar: identification number of toolbar (needed for refresh)
 * @param           [in] ptbd: toolbar data
 * @param           [in] hbmpToolbar: handle to loaded bitmap
 * @return          
 * @exception       -
 * @see             
*/
BOOL CCoolMenuManager::LoadToolbarFromStruct(UINT nIDToolbar, TOOLBARDATA* ptbd, HBITMAP hbmToolbar)
{
	CBitmap bmToolbar;
	...
	BOOL bSelected, HBITMAP hbmCheck)
{
	...
	DeleteObject(hbmCheck);		// Fixed resource leak
	...
	UINT nIndex, BOOL bSysMenu, BOOL bShowButtons)
{
    ...
    // in some rare cases, the menu is not valid any more
    // memory leaks may occur then.
    // return silent.
    if(!pMenu)
    {
        return;
		...
		if (bShowButtons) {
			...
			if (!(info.fType & MFT_OWNERDRAW)) {
				...
				if (!pmd) {									// if no item data:
                    ...
                    // RF: info.wID is >= 0x10000 if menu item is a popup
                    // menu. However GetButtonIndex parameter type is WORD
                    // (16 Bit) and then inside GetButtonIndex the truncated
                    // low word part of info.wID is used which may or may
                    // not deliver a valid button index (sometimes a bitmap
                    // is drawn with popup menus and sometimes not). Since
                    // we never want a popup menu to have a bitmap we just
                    // skip looking up.

#ifdef ORIGINAL_VERSION
					pmd->iButton = GetButtonIndex(info.wID);
#else
                    if ((info.wID & 0xffff0000) == 0)
                    {
					    pmd->iButton = GetButtonIndex(info.wID & 0xffff);
                    }
                    else
                    {
                        pmd->iButton = -1;
...
BOOL CCoolMenuManager::AppendAccelName(CString& sItemName, UINT nID)
{
    ...
    // RF: nID is >= 0x10000 if menu item is a popup menu. However
    // GetAccel parameter type is WORD (16 Bit) and then inside
    // GetAccel the truncated low word part of nID is used which
    // may or may not deliver an accelerator (sometimes an accele-
    // rator text is drawn with popup menus and sometimes not).
    // Since we never want a popup menu to have an accelerator we
    // just skip appending an accelerator.
...
void CCoolMenuManager::FixMFCDotBitmap()
{
	...
	if (hbmDot) {
		...
		DeleteObject(hbmDot);	// SIS: removed resource leak
	}
}
...
//------------------------------------------------------------------*
/**
 * load sys color bitmap from file.
 *
 * @param           [in] lpBitmapFileName: name of bitmap file
 * @param           [in] bMono: monochrome?
 * @return          handle to bitmap
 * @see             LoadSysColorBitmap() above
*/
HBITMAP PxLib::LoadSysColorBitmapFromFile(LPCTSTR lpBitmapFileName, BOOL bMono)
{
    HBITMAP h_bmpImage = (HBITMAP)::LoadImage(0, lpBitmapFileName, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
    return h_bmpImage;
}

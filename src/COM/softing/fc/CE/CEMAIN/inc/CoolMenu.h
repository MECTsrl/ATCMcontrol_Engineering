...
#ifndef __COOLMENU_H_
#define __COOLMENU_H_
...
extern HBITMAP LoadSysColorBitmapFromFile(LPCTSTR lpBitmapFileName, BOOL bMono=FALSE);
} // end namespace
...
// SIS, 07.05.01: for runtime toolbar data
class CToolbarData
{
public:
    CToolbarData();
    ...
    void SetToolbarId(UINT uiID);
    void SetBackgroundColor(COLORREF bgColor);
    void AddToolbarButton(UINT uiItemID, const CString& rstrBitmapFileName);

    int GetNumButtons();

protected:
    UINT                m_uiID;                 // toolbar id
    COLORREF            m_bgColor;              // background color
    CArray<UINT, UINT>  m_ItemIdArray;          // array of item ids
    CStringArray        m_astrBitmapFileName;   // array of bitmap file names

    friend class CCoolMenuManager;
...
public:
    ...
    BOOL LoadToolbarFromStruct(UINT nIDToolbar, TOOLBARDATA* ptbd, HBITMAP hbmToolbar);
    // SIS, 07.05.01: load runtime toolbar
    BOOL AddRuntimeToolbar(CToolbarData* pToolbarData);
    // SIS, 07.05.01: get runtime toolbar data from id (for refresh)
    CToolbarData* GetRuntimeToolbarData(UINT nIDToolbar);
...
protected:
    ...
    // SIS, 07.05.01: list of run time tool bar data structures
    CList<CToolbarData*, CToolbarData*>   m_tToolbarDataList;

	// helpers
	void DestroyAccel();
    void DestroyRuntimeToolbarData();
